...
public abstract class JSONSanitizationSerializer<T> extends StdSerializer<T> {
    ...
    protected void serializeNode(final JsonGenerator jgen, final JsonNode node, final ObjectMapper mapper)
        ...
        while (nodeFieldIterator.hasNext()) {
            ...
            if (nodeValue.isValueNode()) {
                writePrimativeObjectField(jgen, nodeKey, nodeValue);
            } else { // handle complex types (either POJO/JSON Arrays/Objects)
                writeComplexNodeObjectField(jgen, mapper, nodeKey, nodeValue);
            }
        }
    ...
    private void serializeListItems(final JsonGenerator jgen, final JsonNode nodeList, final ObjectMapper mapper,
        ...
        while (nodeIterator.hasNext()) {
            ...
            if (node.isValueNode()) {
                writePrimativeArrayMember(jgen, node);
            } else { // handle complex types (either POJO/JSON Arrays/Objects)
                writeComplexNodeArrayMember(jgen, mapper, nodeKey, node);
            }
        }
    }
     ...
     * @throws IOException
     */
    private void writePrimativeObjectField(final JsonGenerator jgen, final String nodeKey,
     ...
     * @throws IOException
     */
    private void writeComplexNodeObjectField(final JsonGenerator jgen, final ObjectMapper mapper, final String nodeKey,
        final JsonNode nodeValue) throws IOException {
     ...
     * @throws IOException
     */
    private void writePrimativeArrayMember(final JsonGenerator jgen, final JsonNode node) throws IOException {
        if (node.isTextual()) {
     ...
     * @throws IOException
     */
    private void  writeComplexNodeArrayMember(final JsonGenerator jgen, final ObjectMapper mapper, final String nodeKey,

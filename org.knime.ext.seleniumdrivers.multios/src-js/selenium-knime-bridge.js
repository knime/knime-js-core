...
window.seleniumKnimeBridge = (function () {
    ...
    bridge.executeOnFrame = async function (toExecute) {
        let frame = document.getElementById(FRAME_ID);
        let response = await frame.contentWindow.eval('(async function() {' + toExecute + '})();');
    ...
    bridge.respondToViewRequest = function (responseURL) {
        loadJSONFile(responseURL, function (response) {
            bridge.executeOnFrame('if (KnimeInteractivity) {KnimeInteractivity.respondToViewRequest(JSON.parse(\'' +
                response + '\'));}');
        }, false);
    };
    ...
    bridge.updateResponseMonitor = function (monitorURL) {
        loadJSONFile(monitorURL, function (monitor) {
            bridge.executeOnFrame('if (KnimeInteractivity) {KnimeInteractivity.updateResponseMonitor(JSON.parse(\'' +
                monitor + '\'));}');
        }, false);
    };
